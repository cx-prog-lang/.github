...
Generic functions can be _explicitly instantiated_ ("_specialized_") by manually declaring or defining generic functions with actual types. In this case, they can reuse the body of the generic definitions by specifying `...` for their bodies. The explicitly instantiated generic functions are inserted on the spot, and the generic function's initial storage-class specifier is ignored.

The implicitly instantiated generic functions _preserve_ the storage-class specifier of the generic functions.
